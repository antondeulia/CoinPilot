	...
	async parseTransaction(
		...
		const accountInstruction =
			accountNames.length > 0
				? ` У пользователя есть счета: ${accountNames.join(', ')}. Если в тексте упоминается счёт (предлог "с", "из", "на" + название) — укажи в поле account соответствующее название из списка. Сопоставляй слова даже при неточном написании (например "с революта" → Revolut). Счёт "Вне Wallet" — только для переводов: в поле toAccount указывай "Вне Wallet", если пользователь перевёл куда-то вовне (не указал целевой счёт). Для income/expense поле account никогда не должно быть "Вне Wallet".`
				: ''
